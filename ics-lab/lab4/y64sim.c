...
stat_t nexti(y64sim_t *sim)
{
    ...
    case I_RRMOVQ: /* 2:x regA:regB */
    {
        ...
        {
            set_reg_val(sim->r, GET_REGB(codefun), get_reg_val(sim->r, GET_REGA(codefun)));
        }
    ...
    case I_IRMOVQ: /* 3:0 F:regB imm */
        set_reg_val(sim->r, GET_REGB(codefun), imm);
        sim->pc = next_pc;
        break;
    case I_RMMOVQ: /* 4:0 regA:regB imm */
    {
        
        set_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + imm, get_reg_val(sim->r, GET_REGA(codefun)));
        sim->pc = next_pc;
        break;
    }
    case I_MRMOVQ: /* 5:0 regB:regA imm */
    {
        if (!get_long_val(sim->m, get_reg_val(sim->r, GET_REGB(codefun)) + imm, &tmpmem))
        {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, get_reg_val(sim->r, GET_REGB(codefun)) + imm);
            return STAT_ADR;
        }
        set_reg_val(sim->r, GET_REGA(codefun), tmpmem);
        sim->pc = next_pc;
        break;
    }
    case I_ALU: /* 6:x regA:regB */
    {
        tmpmem = compute_alu(ifun, get_reg_val(sim->r, GET_REGA(codefun)), get_reg_val(sim->r, GET_REGB(codefun)));
        set_reg_val(sim->r, GET_REGB(codefun), tmpmem);
        sim->cc = compute_cc(ifun, GET_REGA(codefun), GET_REGB(codefun), tmpmem);
        sim->pc = next_pc;
        break;
    }
    case I_JMP: /* 7:x imm */
    ...
    case I_RET: /* 9:0 */
    {
        get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &tmpmem);
        set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) + 8);
        sim->pc = tmpmem;
        break;
    }
    ...
    {
        tmpmem = get_reg_val(sim->r, GET_REGA(codefun));
        set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) - 8);
        if (!set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), tmpmem))
        {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
            return STAT_ADR;
        }
