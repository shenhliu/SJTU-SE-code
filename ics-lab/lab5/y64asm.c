...
int add_symbol(char *name)
{
    ...
    symbol_t*newsym = malloc(sizeof(symbol_t));
    newsym->name = name;
...
void add_reloc(char *name, bin_t *bin)
{
    ...
    reloc_t*tmp = malloc(sizeof(reloc_t));
    tmp->name = name;
    tmp->y64bin = bin;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    if(!IS_REG(*ptr))
        return PARSE_ERR;
    /* find register */
    const reg_t*tmpreg = find_register(*ptr);
    if(tmpreg == NULL)
    {
        return PARSE_ERR;
    }
...
//used to store a long int into binary file(reverse and store)
...
*           call SUM  #invoke SUM function */
    ...
    if(parse_instr(&ptr, &inst) == PARSE_INSTR){
        ...
        case I_IRMOVQ:
            ...
            if(type == PARSE_DIGIT)
                 storeValue(&line->y64bin.codes[2], value, 8);
        ...
        case I_CALL:
            ...
            if(type == PARSE_DIGIT)
                storeValue(&line->y64bin.codes[1], value, 8);
            else if (type == PARSE_SYMBOL)
                add_reloc(name, &line->y64bin);
            break;
        case I_PUSHQ:
        ...
        case I_DIRECTIVE:
        {
            ...
            case D_DATA:
                ...
                if(type == PARSE_DIGIT)
                    storeValue(&line->y64bin.codes[0], value, 8);
...
int relocate(void)
{
    ...
    while (rtmp->next) {
        ...
        case 4:
        case 8: 
                storeValue(&rtmp->y64bin->codes[0],stmp->addr,8);
            break;
        case 9:
            storeValue(&rtmp->y64bin->codes[1],stmp->addr,8);
            break;
        case 10:
            storeValue(&rtmp->y64bin->codes[2],stmp->addr,8);
            break;
        default:
            return -1;
        }
